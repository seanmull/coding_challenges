    ...
    async def handler(request):
        async with ClientSession() as session:
            ...
            while not health_check:
                url, health_check = server_is_available[counter % len(server_is_available)]
                print(f'Current status of server on {url} is {health_check if "Healthy" else "Unhealthy"}')
                if not health_check:
                    attempts += 1
                    counter += 1
                elif attempts > 3:
...
#                         server_is_available[i][1] = response.status == 200
#                 except Exception:
#                     server_is_available[i][1] = False
#                     
#             print(server_is_available)
#             await asyncio.sleep(1)  # Health check interval
...
#         while not health_check:
#             url, health_check = server_is_available[counter % len(server_is_available)]
#             print(f'Current status of server on {url} is {"Healthy" if health_check else "Unhealthy"}')
#             
#             if not health_check:
#                 attempts += 1
#                 counter += 1
